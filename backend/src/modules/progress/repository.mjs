...
import {
  ...
  publishFuelConversionToLockVault,
...
export async function convertFuelToIchor(
  walletAddress,
  courseId,
  fuelAmount,
) {
  if (!fuelAmount || typeof fuelAmount !== 'number' || fuelAmount < 1) {
  ...
  const dbResult = await withTransactionAsWallet(walletAddress, async (client) => {
    const state = await ensureCourseRuntimeState(client, walletAddress, courseId);

    if (state.gauntletActive) {
      await client.query(
        `insert into lesson.fuel_conversion_receipts
         (wallet_address, course_id, conversion_id, converted_at, applied,
          fuel_before, fuel_after, fuel_amount, ichor_gained)
         values ($1, $2, $3, $4::timestamptz, false, $5, $5, $6, 0)`,
        [walletAddress, courseId, conversionId, timestamp, state.fuelCounter, fuelAmount],
      );
      const courseRuntime = await readCourseRuntimeState(client, walletAddress, courseId);
      return { applied: false, reason: 'GAUNTLET_LOCKED', courseRuntime };
    }
    ...
    const fuelAfter = state.fuelCounter - toConvert;
    const ichorGained = toConvert * ICHOR_PER_FUEL;

    await client.query(
      `update lesson.user_course_runtime_state
       set fuel_counter = $3, updated_at = now()
       where wallet_address = $1 and course_id = $2`,
      [walletAddress, courseId, fuelAfter],
    );
    ...
    await client.query(
      `insert into lesson.fuel_conversion_receipts
       (wallet_address, course_id, conversion_id, converted_at, applied,
        fuel_before, fuel_after, fuel_amount, ichor_gained)
       values ($1, $2, $3, $4::timestamptz, true, $5, $6, $7, $8)`,
      [walletAddress, courseId, conversionId, timestamp,
       state.fuelCounter, fuelAfter, toConvert, ichorGained],
    );

    const courseRuntime = await readCourseRuntimeState(client, walletAddress, courseId);
    return { applied: true, reason: 'CONVERTED', toConvert, ichorGained, courseRuntime };
  });
  ...
  if (!dbResult.applied) {
    return {
      ...
      courseRuntime: dbResult.courseRuntime,
    };
  }

  // Publish on-chain (best-effort — DB is already committed)
  let chain = null;
  if (hasLockVaultRelayConfig()) {
    try {
      await query(
        `update lesson.fuel_conversion_receipts
         set lock_vault_status = 'publishing'
         where wallet_address = $1 and course_id = $2 and conversion_id = $3`,
        [walletAddress, courseId, conversionId],
      );
      const publishResult = await publishFuelConversionToLockVault({
        walletAddress,
        courseId,
        conversionId,
        fuelAmount: dbResult.toConvert,
      });
      await query(
        `update lesson.fuel_conversion_receipts
         set lock_vault_status = 'published',
             ...
             lock_vault_transaction_signature = $4
         where wallet_address = $1 and course_id = $2 and conversion_id = $3`,
        [walletAddress, courseId, conversionId, publishResult.signature],
      );
      chain = { signature: publishResult.signature, lockAccount: publishResult.lockAccount };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      await query(
        `update lesson.fuel_conversion_receipts
         set lock_vault_status = 'failed', lock_vault_last_error = $4
         where wallet_address = $1 and course_id = $2 and conversion_id = $3`,
        [walletAddress, courseId, conversionId, message],
      ).catch(() => {});
      chain = { error: message };
    }
  }
  ...
  return {
    conversionId,
    applied: true,
    fuelConverted: dbResult.toConvert,
    ichorGained: dbResult.ichorGained,
    reason: 'CONVERTED',
    courseRuntime: dbResult.courseRuntime,
    chain,
  };
