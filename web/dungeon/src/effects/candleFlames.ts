...
export function addCandleLight(scene: Scene, position: Vector3, name: string) {
  const light = new PointLight(`${name}_light`, position.add(new Vector3(0, 0.5, 0)), scene);
...
export function addChandelierGlow(scene: Scene, position: Vector3, name: string) {
  const light = new PointLight(`${name}_light`, position.add(new Vector3(0, -0.3, 0)), scene);
